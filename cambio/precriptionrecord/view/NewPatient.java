...
public class NewPatient extends JInternalFrame{
	...
	private void addLabel(){
		...
		JLabel lProfilePicture = new JLabel("Profile Picture");
		...
		constraintsLabel.gridx = 0;
		constraintsLabel.gridy = 0;
		gridbag.setConstraints(lSurname, constraintsLabel);
		add(lSurname);
	...
	private void addField(){
		...
		gridbag.setConstraints(tSurname, constraintsField);
		add(tSurname);
	...
	private void saveButtonAction(){
		bSave.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Date birthday = null;;
				try {
					birthday = new SimpleDateFormat("dd/MM/yyyy").parse(tBirthday.getText());
				} catch (ParseException e1) {
					e1.printStackTrace();
				}
				String telephone = tTp.getText();
				String medicalHistory = tMedicalHitory.getText();
				Patient patient = new Patient(name,surName,address,gender,status,birthday,telephone,medicalHistory);
				new DBConnection();
			}
		});
	}
